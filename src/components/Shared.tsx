...
export const EventCard: React.FC<{ event: Event }> = ({ event }) => (
  <div className="bg-white rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col h-full group border border-gray-100">
    <div className="relative h-48 overflow-hidden">
      ...
      />
      <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-bold text-vci-yellow uppercase tracking-wide">
        {event.difficulty}
      </div>
    </div>
    <div className="p-6 flex-grow flex flex-col">
      ...
      <h3 className="text-xl font-serif font-bold text-gray-800 mb-2 group-hover:text-vci-yellow transition-colors">{event.title}</h3>
      ...
      <div className="mt-auto pt-4 border-t border-gray-100 flex items-center justify-between">
        ...
        <Link to="/partecipa" className="text-vci-yellow font-semibold text-sm flex items-center gap-1 hover:gap-2 transition-all">
